  ...
  const articleList = articles.map(article => (
    <li key={article.id}>
      <Link to={`/p/${article.id}`} className="block h-40 relative">
        ...
        {article.images.length > 1 && ( // 이미지가 여러장인 경우 썸네일에 표시
          <svg id="Layer_1" height="20" fill="white" viewBox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg" data-name="Layer 1" className="absolute top-2 right-2">
